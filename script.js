...
const SUPABASE_KEY = "sb_publishable_VVxY5XJkiEn5ojE9UY9uXA_fg9djb70";
...
async function reviewWhatsApp(form){
  ...
  const name =
    String(fd.get('reviewName') || 'Anonymous').trim();

  const review =
    String(fd.get('review') || '').trim();
  ...
  const msg =
`Adeel Jaffar Video Editor Review
Rating: ${stars}/5
Name: ${name}
Review: ${review}`;
  ...
  const thanks =
    document.getElementById('reviewThanks');

  if(thanks){

    thanks.textContent =
      "Thank you! Your review is now visible to everyone on the website. ⭐";
  }
